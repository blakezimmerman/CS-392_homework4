...
int main(int argc, char **argv)
{
  ...
  // Validate args
  if (!is_power_of_two(argv[1]))
  {
    fprintf(stderr, "matrix_width must be a power of 2\n");
    return -1;
  }
  if (!is_power_of_two(argv[2]))
  {
  ...
  // Allocate matrices
  float *input = (float *)malloc(matrix_width * matrix_width * sizeof(float));
  float *output = (float *)malloc(matrix_width * matrix_width * sizeof(float));
  ...
  for (int i = 0; i < matrix_width; i += block_width)
  {
    for (int j = 0; j < matrix_width; j += block_width)
    {
      for (int k = i; k < i + block_width; ++k)
      {
        ...
        {
          output[k + l * matrix_width] = input[l + k * matrix_width];
        }
      }
    }
  }
